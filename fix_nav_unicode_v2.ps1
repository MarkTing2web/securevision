    ...
    <li class="nav-item"><a href="contact.html" class="nav-link has-dropdown">Contact</a>
      <div class="mega-menu mega-menu-contact">
        ...
        <div class="mega-links-col">
          ...
          <a href="https://wa.me/6593860466" class="contact-ch-link whatsapp-ch"><span class="cch-icon">$wa</span><div class="cch-text"><div class="cch-label">WhatsApp $arrow Fastest</div><div class="cch-sub">+65 9386 0466</div></div></a>
          ...
          <div class="contact-hours">$clock Mon$arrowFri 8:00$arrow17:00 $middot Sat by appt</div>
        </div>
      </div>
    </li>
  </ul>
...
foreach ($file in $files) {
    ...
    # FIX MOJIBAKE FIRST (Global replacements)
    $content = $content.Replace("â„¢", $tm)
    $content = $content.Replace("Â·", $middot)
    $content = $content.Replace("â€”", [char]0x2014) # em dash
    $content = $content.Replace("â€“", [char]0x2013) # en dash
    $content = $content.Replace("Â©", [char]0x00A9) # ©
    $content = $content.Replace("Â®", [char]0x00AE) # ®
    $content = $content.Replace("â†’", $arrow)
    $content = $content.Replace("âœ✓", $check)
    $content = $content.Replace("âœ", $check)
    $content = $content.Replace("â–¾", $tri)
