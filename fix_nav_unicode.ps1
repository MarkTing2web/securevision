...
$houses = Get-UnicodeChar "1F3D8" # 🏘️
...
$tag = Get-UnicodeChar "1F3F7" # 🏷️
$folder = Get-UnicodeChar "1F4C2" # 📂
$calc = Get-UnicodeChar "1F9AE" # 🧮
$books = Get-UnicodeChar "1F4DA" # 📚
$wrench = Get-UnicodeChar "1F527" # 🔧
$inbox = Get-UnicodeChar "1F4E5" # 📥
$crane = Get-UnicodeChar "1F3D7" # 🏗️
...
$shield = Get-UnicodeChar "1F6E1" # 🛡️
$trophy = Get-UnicodeChar "1F3C6" # 🏆
$clipboard = Get-UnicodeChar "1F4CB" # 📋
  ...
  <ul class="nav-menu">
    ...
    <li class="nav-item"><a href="technology.html" class="nav-link has-dropdown">Technology</a>
      <div class="mega-menu mega-menu-technology">
        <div class="mega-guidance"><div class="mega-col-label">Our Systems</div><h4>Systems &amp; Technology</h4><p>Four subsystem groups on the SECURE™ framework.</p><a href="technology.html" class="view-all-link">$arrow View All Technology</a><div class="mega-trust"><div class="trust-item">$check SECURE™ Integration</div><div class="trust-item">$check VESTA™ Enterprise Platform</div><div class="trust-item">$check GantryGo™ Vehicle Intercom</div></div></div>
      ...
      <div class="mega-menu mega-menu-contact">
        <div class="mega-links-col">
          ...
          <div class="about-preview-card">
            <div class="apc-founder"><div class="apc-avatar">$user</div><div><div class="apc-name">Ler Wee Meng</div><div class="apc-title">Founder · NUS Eng + UOL Law</div></div></div>
        ...
        <div class="mega-links-col">
          ...
          <a href="https://wa.me/6593860466" class="contact-ch-link whatsapp-ch"><span class="cch-icon">$wa</span><div class="cch-text"><div class="cch-label">WhatsApp — Fastest</div><div class="cch-sub">+65 9386 0466</div></div></a>
          ...
          <div class="contact-hours">$clock Mon–Fri 8:00–17:00 · Sat by appt</div>
        </div>
      </div>
    </li>
  </ul>
...
@media (max-width: 1200px) {
  .mega-menu{left: auto; right: 0; transform: none; width: auto; max-width: calc(100vw - 40px);}
  .mega-menu-technology{width: 900px;}
}
...
foreach ($file in $files) {
    ...
    # Replace CSS
    if ($content -match '(?s)<style id="sv-nav-css">.*?</style>') {
    ...
    if ($originalContent -ne $content) {
        ...
        Write-Host "Fixed or Updated $($file.Name)"
    }
}
