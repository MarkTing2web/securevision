...
$shield = Get-UnicodeChar "1F6E1" + [char]0xFE0F # 🛡️
$trophy = Get-UnicodeChar "1F3C6" # 🏆
$link = Get-UnicodeChar "1F517" # 🔗
$star = Get-UnicodeChar "2B50" # ⭐
$wrench = Get-UnicodeChar "1F6E0" + [char]0xFE0F # 🛠️
...
# 1. Surgical Replacements for specific mangled icons in body
$content = $content.Replace("🛡ï¸ ", $shield)
$content = $content.Replace("ðŸ †", $trophy)
$content = $content.Replace("ðŸ”—", $link)
$content = $content.Replace("â­ ", $star)
$content = $content.Replace("ðŸ› ï¸ ", $wrench)
$content = $content.Replace("ðŸ’°", $money)
$content = $content.Replace("ðŸ’¬", $wa)
$content = $content.Replace("ðŸ“ž", $tel)
$content = $content.Replace("â”€â”€", "$([char]0x2500)$([char]0x2500)")
$content = $content.Replace("✓“", $check)
$content = $content.Replace("SECUREâ„¢", "SECURE$tm")

# 2. Complete Header Replacement (Consistent with others)
$newNav = @"
<nav class="main-nav">
...
# Replace the OLD style block (which doesn't have the ID)
# It starts on line 13 and seems to go quite long. We'll use regex to find the first style block in head.
if ($content -match '(?s)<style>.*?</style>') {
    $content = [regex]::Replace($content, '(?s)<style>.*?</style>', $newCssHeader)
}

# Replace Nav (Complete)
...
Write-Host "Homepage index.html fixed and modernized."
