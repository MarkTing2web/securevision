...
$emdash = Get-UnicodeChar "2014" # —
$endash = Get-UnicodeChar "2013" # –
    ...
    <li class="nav-item"><a href="contact.html" class="nav-link has-dropdown">Contact</a>
      <div class="mega-menu mega-menu-contact">
        ...
        <div class="mega-links-col">
          ...
          <a href="https://wa.me/6593860466" class="contact-ch-link whatsapp-ch"><span class="cch-icon">$wa</span><div class="cch-text"><div class="cch-label">WhatsApp $emdash Fastest</div><div class="cch-sub">+65 9386 0466</div></div></a>
          ...
          <div class="contact-hours">$clock Mon$endashFri 8:00$endash17:00 $middot Sat by appt</div>
        </div>
      </div>
    </li>
  </ul>
...
foreach ($file in $files) {
    ...
    # CLEAN MOJIBAKE (Robust addition)
    $content = $content.Replace("$([char]0x00E2)$([char]0x201e)$([char]0x00a2)", [string]$tm)
    $content = $content.Replace("$([char]0x00c2)$([char]0x00b7)", [string]$middot)
    $content = $content.Replace("$([char]0x00e2)$([char]0x20ac)$([char]0x201d)", [string]$emdash)
    $content = $content.Replace("$([char]0x00e2)$([char]0x20ac)$([char]0x2013)", [string]$endash)
    $content = $content.Replace("$([char]0x00c2)$([char]0x00a9)", [string]$copyright)
    $content = $content.Replace("$([char]0x00e2)$([char]0x2020)$([char]0x2012)", [string]$arrow)
    $content = $content.Replace("$([char]0x00e2)$([char]0x2013)$([char]0x00be)", [string]$tri)

    # Replace CSS
    if ($content -match '(?s)<style id="sv-nav-css">.*?</style>') {
        $content = [regex]::Replace($content, '(?s)<style id="sv-nav-css">.*?</style>', $newCss)
